 ...
 * ListenerProtocol
 * 装饰器模式、责任链模式
 * 把通用的抽象逻辑进行封装或对子类进行增强，让子类可以更加专注具体的实现
 * 自动包装扩展类 ：这种Wrapper无具体实现，只是做了同样逻辑的抽象，需要在构造方法中传入具体的扩展接口。
 */
public class ProtocolFilterWrapper implements Protocol {

    private final Protocol protocol;
    //实现了Protocol，但构造方法中传入Protocol类型的具体的扩展接口。
    ...
    private static <T> Invoker<T> buildInvokerChain(final Invoker<T> invoker, String key, String group) {
        ...
        if (!filters.isEmpty()) {
            for (int i = filters.size() - 1; i >= 0; i--) {
                ...
                //TODO ②把真实的Invoker放到拦截器的末尾
                final Invoker<T> next = last;
                //TODO ③ 为每个filter生成一个exporter
                last = new Invoker<T>() {
    ...
    public <T> Exporter<T> export(Invoker<T> invoker) throws RpcException {
        ...
        //TODO ① 构造拦截器链（过滤provider分组），然后触发Dubbo协议暴露
