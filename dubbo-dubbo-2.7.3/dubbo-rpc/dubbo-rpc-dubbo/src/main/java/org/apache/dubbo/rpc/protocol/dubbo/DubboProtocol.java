...
public class DubboProtocol extends AbstractProtocol {
    ...
    public <T> Exporter<T> export(Invoker<T> invoker) throws RpcException {
        ...
        //TODO ①获取服务标识构建key，理解成服务坐标也行。由服务组名，服务名，服务版本号以及端口组成。比如：
        // demoGroup/com.alibaba.dubbo.demo.DemoService:1.0.1:20880
        // export service.
        String key = serviceKey(url);
        // 创建 DubboExporter
        DubboExporter<T> exporter = new DubboExporter<T>(invoker, key, exporterMap);
        // TODO ② 将 <key, exporter> 键值对放入DubboProtocol缓存中
    ...
    private ExchangeServer createServer(URL url) {
        ...
        try {
            // TODO 创建服务器实例  创建 ExchangeServer
    ...
    /**
     * TODO  DubboProtocol 的 refer ->  protocolBindingRefer
     * @param serviceType
     * @param url
     * @param <T>
     * @return
    ...
    public <T> Invoker<T> protocolBindingRefer(Class<T> serviceType, URL url) throws RpcException {
        ...
        // 比如 NettyClient、MinaClient 等，默认情况下，Dubbo 使用 NettyClient 进行通信。接下来，我们简单看一下 getClients 方法的逻辑。
     ...
     * TODO initClient 方法首先获取用户配置的客户端类型，默认为 netty。
     *   然后检测用户配置的客户端类型是否存在，不存在则抛出异常。最后根据 lazy 配置决定创建什么类型的客户端。
     *   这里的 LazyConnectExchangeClient 代码并不是很复杂，
     *   该类会在 request 方法被调用时通过 Exchangers 的 connect 方法创建 ExchangeClient 客户端，
     *   该类的代码本节就不分析了。下面我们分析一下 Exchangers 的 connect 方法。
     * @param url
     */
    private ExchangeClient initClient(URL url) {
        ...
        try {
            ...
            if (url.getParameter(LAZY_CONNECT_KEY, false)) {
                ...
                // TODO 入口 创建懒加载 ExchangeClient 实例
