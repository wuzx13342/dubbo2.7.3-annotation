...
public class ReferenceConfig<T> extends AbstractReferenceConfig {
    ...
    public synchronized T get() {
        ...
        if (ref == null) {
            // TODO 服务消费入口 init 方法主要用于处理配置，以及调用 createProxy 生成代理类
            init();
        }
        return ref;
    }
    ...
    /**
     * TODO  1.首先根据配置检查是否为本地调用，若是，则调用 InjvmProtocol 的 refer 方法生成 InjvmInvoker 实例。
     *       2.若不是，则读取直连配置项，或注册中心 url，并将读取到的 url 存储到 urls 中。
     *       3.然后根据 urls 元素数量进行后续操作。若 urls 元素数量为1，则直接通过 Protocol 自适应拓展类构建 Invoker 实例接口。
     *       4.若 urls 元素数量大于1，即存在多个注册中心或服务直连 url，此时先根据 url 构建 Invoker。
     *       5.然后再通过 Cluster 合并多个 Invoker，最后调用 ProxyFactory 生成代理类。
     * @param map
     * @return
     */
    ...
    private T createProxy(Map<String, String> map) {
        ...
        if (shouldJvmRefer(map)) {
            ...
            // TODO 单个注册中心或服务提供者(服务直连，下同) 单注册中心消费
            if (urls.size() == 1) {
                ...
                for (URL url : urls) {
                    ...
                    // TODO 入口 根据 url 协议头加载指定的 Protocol 实例，并调用实例的 refer 方法
                    //  Invoker 是 Dubbo 的核心模型，代表一个可执行体。
                    //  在服务提供方，Invoker 用于调用服务提供类。
                    //  在服务消费方，Invoker 用于执行远程调用。
                    //  Invoker 是由 Protocol 实现类构建而来。
                    //  Protocol 实现类有很多，分别是 RegistryProtocol 和 DubboProtocol等
        ...
        // create service proxy      TODO // 生成代理类  把Invoker转换成接口代理
